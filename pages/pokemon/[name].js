...
export default function PokemonDetailPage({ pokemon }) {
  ...
  return (
    <div>
      <Head>
        <title>Pokedex - {pokemon.name} type and stats</title>
        <meta name="description" content={`Pokemon ${pokemon.name} types, stats details. Find another Pokemon only on Pokedex!`} />
      </Head>
      <main>
        <header>
...
export async function getStaticProps({ params }) {
  ...
  return {
    ...
    revalidate: 15 * 60 // 15 minutes (maybe longer is better because supposedly pokemon data does not change that much)
  }
}
